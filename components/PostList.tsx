...
const PostList = ({  posts,  loading = false, postsEnd = false, enableLoadMore = false }) => {
    ...
    return posts ? posts.map((post, index, array) => {
        ...
        return (<>
            ...
            enableLoadMore && (
                <>
                    <Link className="flex py-16 hover:py-14 min-w-fit" href={`/${post.username}/${post.slug}`} key={post.slug}>
                        <div className="w-64 p-4 hover:px-8 flex lg:mx-0 mx-3 bg-blog-white dark:bg-fun-blue-600 dark:text-blog-white hover:rounded-3xl rounded-3xl drop-shadow-lg hover:drop-shadow-xl hover:brightness-125">
